...
![parallel_connection](../images/parallel_connection.png)

순차적으로 object를 가져오는 것이 느리기 때문에 embedded 된 이미지를 parallel 하게 호출하여 속도를 개선한다. HTML 같이 다양한 object가 조합디었다면, dead time 과 대역폭 제한을 사용해 이득을 볼 수 있다.

단일 연결은 한계까지 가득차지 않았다면, client 인터넷의 사용하지 않은 대역폭을 다른 object에 할당 할 수 있다.

이미지가 로드되는 동안 시간이 필요한데 여러개 object는 이 시간을 중첩해 불러오기 때문에 더 효율적이다.

### But Paralled Connection Are Not Always Faster

그러나, 항상 parallel 이 빠른것은 아니다, client의 bandwidth가 부족하다면 data를 전송하는데 모든 시간을 사용할 수 있다. 이런경우 성능상 이점은 없다.

그리고 많은 connection들은 높은 memory를 필요로 한다. 만약 이미지가 1000개 라면 1000개 connection이 필요하게 되고, 이런 요청을 하는 유저가 많다면 서버는 그 만큼의 connection을 견뎌야 하며, 중간에 proxy라도 있다면 그 만큼 부하는 가중된다.

실제 서버에서는 parallel connection을 사용하지만 최대 connection 갯수를 설정하여 사용하며(4개), 많은 요청이 올 경우 끊을 수 있어야 한다.

### Paralled Connections May "Feel" Faster

paralled connection 항상 빠르지 않더라도 대체로 빠르다고 느껴지는 것은 심리적 요인이다. 화면에 여러 object 들이 동시에 움직이는게 보이기 때문이다.

### Persistent Connection

web client는 보통 여러개 connection 하나의 site로 향한다(site locality) 이미지는 역시 같은 server에 받아오고, hyper link도 같은 site로 향하는 경우가 많다.

이런 이유로 HTTP/1.1은 device에 TCP connection을 유지하고 가까운 미래에 재사용 하도록 되어있다. (HTTP request)

TCP Connection은 transaction 뒤에도 열려 있도록 하는것이 Persistent connection 이다. Persistent 하지 않은 ㅇ녀결은 transaction 뒤에 연결을 끊는다. 이미 TCP가 연결되어 있으니 slow start는 건너 뛴다. 따라서 data 전송이 빠르다.

### Persistent Versue Parallel Connections

parallel 과 비교했을 때, composite page는 빠르다. 그러나 paralled 은 아래와 같은 단점이 있다.

- 각각의 transaction에 open/close가 존재해 대역폭과 시간의 비용이 발생한다.
- 각각의 새로운 TCP connection은 slow start가 발생해 성능을 낮춘다.
- parallel connection open에 제한이 있다.

반면 Persistent connection은 delay와 overhead를 줄이며 connection이 "tuned" 상태이며, open conneciton의 갯수를 줄일 수 있다.

하지만 persistent connection은 관리가 필요하다. idle connection이 쌓이지 않아야 하며, local과 client server 모두에 resource가 필요하다. 그래서 Persistent Connection과 Parallel connection을 함께 사용하는것이 효율적이다. 

HTTP/1.0 에서는 keep-alive가 persistent connection을 대체하여 사용한다.

### HTTP/1.0 + Keep-Alive Connections

HTTP/1.0은 Persistent Connection에 해당하는 Keep-Alive 를 실험적인 목적으로 사용했다. (1996) 이 이른 Persistent Connection은 후에 HTTP/1.1과 호환할 때 상당히 개발자들을 괴롭게 했으나, 현재 많은 client가 keep-alive를 사용하고 있다.

![keep-alive](../images/keep_alive.png)

특징은 serial connection과 대체로 동일하지만 time line을 보면 connection과 close에서 오는 overhead를 remove 했다.

### Keep-Alive Operation

Keep-Alive가 deprecated 되어 HTTP/1.1 spec doc에서는 아예 삭제되었지만, hand shake 과정에서 여전히 사용된다.

![keep-alive_handshake](../images/keep-alive_handshake.png)

request시 header에 Keep-Alive를 추가하여 전송한다. 서버가 지속적으로 Keep-Alive를 사용한다면 response header에 Keep-Alive를 추가하여 응답하면 된다.
만약 이 Header가 없다면 client는 server가 Keep-Alive를 지원하지 못한다고 판단한다.
