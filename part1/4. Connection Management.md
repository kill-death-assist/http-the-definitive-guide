...
하지만 persistent connection은 관리가 필요하다. idle connection이 쌓이지 않아야 하며, local과 client server 모두에 resource가 필요하다.

그래서 Persistent Connection과 Parallel connection을 함께 사용하는것이 효율적이다. 
